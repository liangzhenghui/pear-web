...
import { Form, Input, Button, Alert, Divider, Table, Row, Col, Tag, Icon, notification, Popover, Spin, Select, List, Modal, Tooltip } from 'antd';
...
class Step2 extends React.PureComponent {
  ...
  filterCities = (event) => {
    ...
    const newCities = cities.filter(item => item.name.indexOf(cityKey) > -1 || item.pinyin.indexOf(cityKey) > -1)
    this.setState({
  ...
  render() {
    const { dispatch,
      searchRestaurantLoading, choiceReataurantNameLoading, commitTaskLoading, fetchCitiesLoading,
      restaurantArea, restaurantListOfArea, cities } = this.props;
    const { selectedArea, selectedRestaurants, restaurantsPagination, filterCities, selectedCity, visibleCityListPop } = this.state
    ...
    const choiceArea = [{
      ...
      key: 'city',
      width: 300,
      render: (text, record) => <div className={selectedArea && record.id === selectedArea.id ? styles.selectedArea : styles.chioceList}>
        <p style={{ color: "#333", marginBottom: 2 }}>{record.name}</p>
        <p style={{ fontSize: 8, color: "#999", marginBottom: 0 }}>{record.address}</p>
      </div>
    }];
    ...
    return (
      <Form layout="horizontal" className={styles.stepForm}
        ...
        {searchRestaurantLoading && <Form.Item><Spin /></Form.Item>}
        <Form.Item>
          <Row gutter={16}>
            ...
            {restaurantArea.length > 0 & restaurantListOfArea.length > 0 &&
              <Col span={15}>
                ...
                <Table
                  ...
                  footer={() => <Button style={{ margin: "0 141px" }}
        ...
        <Form.Item style={{ margin: "0 269px" }} >
          <Button type="primary" onClick={this.commitTask} loading={commitTaskLoading} style={{ marginRight: 16 }}>提交</Button>
          <Button onClick={onPrev} style={{ marginLeft: 8 }}>上一步</Button>
        </Form.Item>
      </Form>
    );
  }
